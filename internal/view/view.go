...
// brief, the status report and the generated board.
package view
...
// Board renders the generated .forge/BOARD.md.
func Board(p *project.Project) string {
	var b strings.Builder
	b.WriteString("# Board\n\n")
	b.WriteString("Generated by `forge board`. Do not edit by hand and do not commit it:\n")
	b.WriteString("it is a projection of the specs, and the truth is each spec's frontmatter.\n\n")
	b.WriteString("| Spec | Status | Waiting on | Title |\n|---|---|---|---|\n")
	for _, s := range p.Specs {
		fmt.Fprintf(&b, "| %s | %s | %s | %s |\n", s.ID, s.Status,
			workflow.WaitingFor(s.Status), s.Title)
	}
	if len(p.Specs) == 0 {
		b.WriteString("| — | — | — | — |\n")
	}
